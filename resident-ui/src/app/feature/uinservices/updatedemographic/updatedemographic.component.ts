  ...
  buildData(){
    for (var schema of this.schema['identity']) {
      ...
      if(this.userInfo[schema.id]){
        if (typeof this.userInfo[schema.id] === "string") {  
          this.buildJSONData[schema.id] = this.userInfo[schema.id];
          console.log("this.userInfo[schema.id]34>>>"+this.userInfo[schema.id])
        }else{
          this.buildJSONData[schema.id] = this.userInfo[schema.id][0].value;
          console.log("this.userInfo[schema.id]12>>>"+JSON.stringify(this.userInfo[schema.id][0].value));          
        }
      }      
    }
  }
